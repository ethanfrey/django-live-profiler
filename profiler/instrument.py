...
def wrap_mongo(orig, query, instrument_cursor=False):
...
def find_action(collection, spec={}, *args, **kwargs):
    log_spec = spec.copy()
    for key in log_spec.keys():
        if not key == "_cls":
            log_spec[key] = '?'
    return u"{} : {}".format(collection.name, log_spec)
...
def instrument_mongo():
    try:
        ...
        Collection.find = wrap_mongo(Collection.find, find_action)
    except ImportError:
        pass
